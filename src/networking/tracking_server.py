...
class TrackingServerManager:
    ...
    async def update_position(self, new_position=None):
        ...
        params_list = [{"x": int(x * 100), "y": int(y * 100)}]
        url = f"{self.url}/pos"
        for params in params_list:
            try:
                async with self.http_session.post(url, params=params) as response:
                    ...
                    print(f"Position updated : {message}")
            except Exception as e:
                print(f"ERROR - Tracking server error: {e}")

    async def send_marker(self, marker_id, marker_position):
        if self.TEST_MODE:
