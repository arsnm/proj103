...
run_production_test() {
    ...
    tmux send-keys -t artefact:0 "python -m src.main" C-m
...
run_camera_tests() {
    ...
    python -m src.vision.aruco_detector.py "$1"
}

run_motors_tests() {
    run_config_env
    echo "Running movement tests..."
    python -m src.motor.motor_controller.py "$1"
}

run_servers_tests() {
    run_config_env
    echo "Running the servers on the specified ports..."
    python -m src.web.server "$1" "$2"
}

run_tracking_server_tests() {
    run_config_env
    echo "Running tracking server tests..."
    python -m src.networking.tracking_server_manager "$1" "$2" "$3" "$4"
}
